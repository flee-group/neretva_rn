...
ner_pts$id_row = 1:nrow(ner_pts)
ner_pts = ner_pts[complete.cases(ner_pts),]
ner_pts$id = 1:nrow(ner_pts)
ner_pts = merge(ner_pts, Tp_df, by.x = "id_row", by.y = "us", all.x = TRUE)

### find outlet, define length from midpoint of pixel out
...
# redefine the downstream pixel ids in terms of the 'new' row numbers, not incluing non-wet pixels
ner_pts$downstream_id = with(ner_pts, id[match(ds, id_row)])

# drop and rename columns, then save
cols = c("id", 'stream', 'x', 'y', 'accum', 'drainage', 'catchment.2', 'length_m', 'downstream_id')
ner_pts = ner_pts[, cols]
colnames(ner_pts)[c(2, 7)] = c("reach_id", "catch_area_km2")
